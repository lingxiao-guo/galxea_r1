...
class EpisodicDatasetGalaxea(torch.utils.data.Dataset):
    def __init__(self, bags_name, dataset_dir, norm_stats, 
                 ...
                 with_torso, with_chassis, image_overlay, one_hot_emb_flag=False):
...
def get_norm_stats_galaxea(dataset_dir, tf_representation, arm_type, with_torso, with_chassis):
    ...
    stats = {"action_mean": action_mean.numpy().astype(np.float32), "action_std": 2*action_std.numpy().astype(np.float32),
...
def load_data(dataset_dir, chunk_size, batch_size_train, batch_size_val, camera_names, tf_representation, arm_type, with_torso, with_chassis, image_overlay, use_one_hot_task=False, train_ratio=0.8):
    ...
    train_dataset = EpisodicDatasetGalaxea(train_bags, dataset_dir, norm_stats, chunk_size, 
                                           camera_names, tf_representation, arm_type, with_torso, with_chassis, image_overlay, one_hot_emb_flag=use_one_hot_task)
